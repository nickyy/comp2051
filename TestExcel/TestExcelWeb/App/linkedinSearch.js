...
function displayLinkedInResults(peopleSearch, locationName) {
    ...
    for (var i = 0; i < members.length; i++) {
        ...
        html += '<li><input type="checkbox" class="checkbox" value="ln0' + i + '"><span><strong id="ln0'+i+'">' + member.firstName + ' ' + member.lastName + '</strong></span></li>';

        // Print member's location if it is defined
        if (member.location !== undefined)
            html += '<li><input type="checkbox" class="checkbox" value="ln1' + i + '"><span id="ln1' + i + '">' + member.location.name + '</span></li>';

        // Print member's headline if it is defined
        if (member.headline !== undefined)
            html += '<li><input type="checkbox" class="checkbox" value="ln2' + i + '"><span id="ln2' + i + '">' + member.headline + '</span></li>';

        // Print member's summary if it is defined
        if (member.summary !== undefined)
            html += '<li><input type="checkbox" class="checkbox" value="ln3' + i + '"><span id="ln3' + i + '">' + member.summary + '</span></li>';
